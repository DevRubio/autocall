...
export const deleteData = async (Table_Name:string, partitionKey: string, rowKey: string) => {    
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ TableName: Table_Name, PartitionKey: partitionKey, RowKey: rowKey  }),
      });
      if (!response.ok) {
